...
export default class Pageres<DestValue: string> extends EventEmitter {
	...
	constructor(options: Options) {
		...
		this.options.increment = this.options.increment || false;
		this.options.defaultDelimiter = this.options.defaultDelimiter ? this.options.defaultDelimiter : false;
	...
	create(uri: string, size: string, options: Options) {
		...
		do {
			count++;
			const defaultNotation = '(' + count + ')';
			stream.filename = filename({
				crop: options.crop ? '-cropped' : '',
				increment: options.increment && count > 0 ? options.defaultDelimiter ? defaultNotation : count : '',
				date: easydate('Y-M-d'),
				time: easydate('h-m-s'),
				size,
				width: sizes[0],
			...
			generatedFileName = stream.filename;
		} while (fs.existsSync(folderPath + '/' + generatedFileName));
