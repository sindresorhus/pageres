...
exports.save = function (streams, cb) {
	...
	eachAsync(streams, function (stream, i, next) {
		mkdir(this.dest(), function (err) {
			...
			var dest = stream.filename.indexOf('/') === 0
				? stream.filename
				: path.join(this.dest(), stream.filename);
...
exports.create = function (uri, size, options) {
	...
	var filenameWithPngExtension = (options.filename.substr(-4) === '.png'
		? options.filename
		: options.filename + '.png');
	var filename = _.template(filenameWithPngExtension, {
