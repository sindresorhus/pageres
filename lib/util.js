...
export function resolution(url, options) {
	const g = memoizeAsync(getRes);

	return pify(g, Promise)().then(items => {
...
export function viewport(obj, options) {
	const v = memoizeAsync(viewportList);

	return pify(v, Promise)(obj.keywords).then(items => {
		for (const item of items) {
...
export function save(streams) {
	const files = [];

	function end() {
		return Promise.all(files.map(file => pify(rimraf, Promise)(file)));
	}

	process.on('SIGINT', () => end().then(process.exit));

	return Promise.all(streams.map(stream => {
		return pify(mkdirp, Promise)(this.dest()).then(() => {
